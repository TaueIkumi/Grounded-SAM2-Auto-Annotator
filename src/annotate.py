...
def prepare_zip_for_upload(format: str, dataset_dir: Path) -> str:
    ...
    if format == "coco":
        ...
        shutil.make_archive(base_name, 'zip', root_dir=str(dataset_dir))
        ...
        zip_filename = "temp_yolo.zip"
        print(f"Creating YOLO ZIP: {zip_filename} ...")
...
def main():
    ...
    parser.add_argument("--format", type=str, required=True, choices=["coco", "pascal_voc", "cvat_xml", "yolo"], help="Format of the dataset")
    ...
    parser.add_argument("--labels", type=str, required=True, help="Comma separated labels (e.g. 'car,dog,person')")
    ...
    label_list = [{"name": l.strip()} for l in args.labels.split(",")]
